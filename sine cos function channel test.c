...
void DAC_isr_ch2(unsigned int );
void DAC_isr_ch3(unsigned int );
void DAC_isr_ch4(unsigned int );
...
float y1[100],y[100],m[100],x1[100],f=1000,vr[100],vy[100],vb[100];
int c=1;
float a=-0.5,b=0.866;
float dt=10e-6; 
...
{	
	...
	*/
	...
	x1[0]=0; y1[0]=1; 
	vr[0]=0;  // Set initial state of harmonic oscillator
	vy[0]=0;
	vb[0]=0;
	while(1)
        {
        	...
        	{
  					// --- Harmonic Oscillator-------//	  
        			x1[c]=x1[c-1]+6.28*f*dt*y1[c-1];
        		    y1[c]=y1[c-1]-6.28*f*dt*x1[c];
        		    vr[c]=(x1[c]+1)*2047*b; //0-1
        		   	vr[c]=floor(vr[c]);
        		 	DAC_isr_ch1(vr[c]);
					
        		    vy[c]=((a*x1[c]-b*y1[c])+1)*2047*b;
        		   	vy[c]=floor(vy[c]);
        		    DAC_isr_ch2(vy[c]);
    				
    				vb[c]=((a*x1[c]+b*y1[c])+1)*2047*b;
    				vb[c]=floor(vb[c]);
    				DAC_isr_ch3(vb[c]);
...
void DAC_isr_ch2(unsigned int x)
{		
	// A0,A1=0 FOR CHANNEL 1
	GpioDataRegs.GPBCLEAR.bit.GPIO46=1;
...
void DAC_isr_ch3(unsigned int x)
{		
	// A0,A1=0 FOR CHANNEL 1
...
void DAC_isr_ch4(unsigned int x)
{		
	// A0,A1=0 FOR CHANNEL 1
	GpioDataRegs.GPBSET.bit.GPIO46=1;
	GpioDataRegs.GPBSET.bit.GPIO47=1;
